namespace MEnglishServer
{
    public class News
    {
        public List<Article> All { get; set; } = new List<Article>
        {
            new Article
            {
                Title = "Профиль",
                Body = "Теперь в разделе [🥇Профиль] можно просматривать прогресс изучения слов. В приложение интегрирована база данных - обеспечено сохранение прогресса на диск. Также, по клику можно добавлять свои собственные, либо изменять и удалять существующие слова. Путь к картинке можно вставлять по ссылке из интернета",
                Date = new DateTime(2022, 12, 2)
            },

            new Article
            {
                Title = "Сброс слов",
                Body = "Добавлены новые слова. Теперь приложение подстраивается под Вас. Знает, какие слова Вам лучше изучить/повторить сейчас, а какие оставить на потом. Для обновления слов зайдите в раздел [🥇Профиль], кликните по любому слову и нажмите [Сбросить слова]",
                Date = new DateTime(2022, 12, 7)
            },

            new Article
            {
                Title = "Сброс слов",
                Body = "Сброс слов в [🥇Профиль] больше не требуется для добавления слов, пришедших с обновлениями! Это было очень непросто",
                Date = new DateTime(2022, 12, 8)
            },

            new Article
            {
                Title = "Добавлен поиск",
                Body = "Добавлен поиск по словам в [\U0001f947Профиль]. Искать можно как по английскому/русскому слову, так и по рейтингу. Обновлен макет новостей на этой странице",
                Date = new DateTime(2022, 12, 9)
            },

            new Article
            {
                Title = "Обновлены иконки",
                Body = "Обновлены иконки панели навигации, добавлены новые слова",
                Date = new DateTime(2022, 12, 11)
            }
        };
    }
}
